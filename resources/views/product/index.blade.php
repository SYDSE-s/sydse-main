...
@section('content')
    ...
    <div class="container wrap-product">
        <div class="feature row gy-3 gy-sm-0 my-2">
            <form action="{{ route('search-product') }}" method="POST" class="search col-md-3">
                @csrf
                <input type="text" name="search-product" placeholder="cari">
                <button type="submit" class="btn btn-white">Search</button>
                {{-- <a href="{{ route('product') }}" class="btn btn-first">Clear Filter</a> --}}
            </form>
            <div class="filter-category d-flex flex-wrap justify-content-start align-items-center gap-1 col-md-9">
        ...
        <div class="row gy-3 justify-content-between">
            <div class="col-md-8 scroll-card">
                <div class="row gy-2">
                    @if (isset($filtered_product))
                        @foreach ($filtered_product as $product)
                            <div class="product-item col-6 col-lg-4" data-category="{{ $product->product_category }}"
                                data-id="{{ $product->id }}">
                                <div class="card ">
